<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600">
            {{ __('Assicurati di usare una password complessa') }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('password.update') }}" class="mt-6 space-y-6">
        ...
        <div class="d-flex align-items-center gap-4">
            <button type="submit" class="btn btn-primary">{{ __('Conferma Modifiche') }}</button>
            ...
            <p id='status' class=" fs-5 text-muted">{{ __('Password Modificata.') }}</p>
            @endif
        </div>
    </form>
</section>
